...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="+alsa +dbus +11x debug g15 oss pch portaudio pulseaudio speech zeroconf"
...
src_configure() {
	...
	use 11x || conf_add="${conf_add} no-11x"
...
src_install() {
	...
	if use 11x; then
		dobin "${dir}"/mumble11x|| die "Installing 1.1.x compatibility client failed."
	fi
	...
	doman man/mumble-overlay.1 || die "Installing mumble-overlay manpage
failed."
